...
function lpad( str, len, pad ) {
	var p;
	if ( !isString( str ) ) {
		throw new TypeError( format( '1Ol3F,Ex', str ) );
	}
	if ( !isNonNegativeInteger( len ) ) {
		throw new TypeError( format( '1Ol3X,F1', len ) );
	}
	if ( arguments.length > 2 ) {
		p = pad;
		if ( !isString( p ) ) {
			throw new TypeError( format( '1OlAO,M6', p ) );
		}
		if ( p.length === 0 ) {
			throw new RangeError( format('1Ol1W,ED') );
		}
	} else {
		p = ' ';
	}
	if ( len > FLOAT64_MAX_SAFE_INTEGER ) {
		throw new RangeError( format( '1OlAP,M7', len ) );
	}
