...
function lpad( str, len, pad ) {
	var p;
	if ( !isString( str ) ) {
		throw new TypeError( format( '1Ol3F', str ) );
	}
	if ( !isNonNegativeInteger( len ) ) {
		throw new TypeError( format( '1Ol3X', len ) );
	}
	if ( arguments.length > 2 ) {
		p = pad;
		if ( !isString( p ) ) {
			throw new TypeError( format( '1OlAO', p ) );
		}
		if ( p.length === 0 ) {
			throw new RangeError( format('1Ol1W') );
		}
	} else {
		p = ' ';
	}
	if ( len > FLOAT64_MAX_SAFE_INTEGER ) {
		throw new RangeError( format( '1OlAP', len ) );
	}
